...
const PulseFlowRateDeviceForm: React.FC<PulseFlowRateDeviceFormProps> = ({
  ...
  const [formState, setFormState] = useState({
    ...
    // Parameters fields
    frequencyType: "LF",
    frequency: lfOptions[0].toString(),
    pulseDuration: "",
    pulsePause: "",
    scalingFactor: "", // New field
    offset: "", // New field
    minFlowrate: "", // New field
    maxFlowrate: "", // New field
  });
  ...
  const handleSubmit = () => {
    const finalConfig = {
      ...
      parameters: {
        ...
        scalingFactor: formState.scalingFactor
          ? parseFloat(formState.scalingFactor)
          : null,
        offset: formState.offset ? parseFloat(formState.offset) : null,
    ...
    console.log("Submitting Formatted Pulse Flow Rate Config:", finalConfig);
    onSave(finalConfig);
  };
        ...
        {activeTab === "parameters" && (
          <div className="grid grid-cols-2 gap-x-6 gap-y-4 animate-fadeIn">
            ...
            <div className="space-y-1">
              ...
              <Input
                value={formState.scalingFactor}
                onChange={(e) =>
                  handleStateChange("scalingFactor", e.target.value)
                }
                placeholder="Please set value"
              />
            </div>
