...
const StreamConfiguration = () => {
  ...
  return (
    // Note: The parent container from the original file is removed.
    // The wrapping `div` is provided by the parent.
    <>
      <div className="bg-white rounded-2xl shadow-md py-2 px-2 border border-gray-200 space-y-8">
        {/* Visualization Section for Desktop */}
          ...
          <div className="flex flex-row flex-wrap justify-around items-center gap-4 sm:gap-8">
            ...
            <div className="flex flex-col items-center">
              ...
              <div className="flex items-center mt-2">
                ...
                <div className="ml-2 text-xs space-y-4 text-black font-semibold">
                  <div>100°C</div><div>50°C</div><div>0°C</div>
                </div>
              </div>
            </div>
            ...
            <div className="flex flex-col items-center">
              ...
              <div className="relative w-36 h-36 rounded-full border-[6px] border-black flex items-center justify-center">
                ...
                <div className="absolute bottom-2 text-sm text-black font-semibold">x10<sup>5</sup>N/m<sup>2</sup></div>
              </div>
            </div>
          </div>
        </div>

        {/* 4-CARD TABLET-ONLY LAYOUT */}
        <div className="md:hidden bg-white rounded-2xl py-4 px-2 border border-gray-200">
          ...
          <div className="flex flex-col sm:flex-row justify-between gap-4 sm:gap-2">
            {["Volume", "Temperature", "Pressure", "Conversion"].map((label) => (
              <div key={label} className="flex-1 border rounded-xl shadow p-4 flex flex-col items-center justify-center min-w-[100px] bg-white">
                <h3 className="text-md font-semibold text-gray-800 mb-4">{label}</h3>
                <div className="flex flex-col items-center space-y-1 text-sm">
                  <div>Positive: <span className="text-green-600 font-semibold">75%</span></div>
                  <div>Negative: <span className="text-red-600 font-semibold">75%</span></div>
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="bg-white rounded-2xl py-6 px-2 border border-gray-200">
          <div className="flex flex-wrap justify-between gap-2 sm:gap-4">
            <button onClick={() => openModal("volume")} className="flex-1 min-w-[70px] sm:min-w-[100px] max-w-[150px] flex items-center justify-center gap-1 sm:gap-2 py-2 px-3 sm:py-3 sm:px-4 text-xs sm:text-sm border rounded-xl shadow-sm hover:bg-gray-100 transition"><List className="text-yellow-500" size={18} /> <span className="font-medium">Volume</span></button>
            <button onClick={() => openModal("temperature")} className="flex-1 min-w-[70px] sm:min-w-[100px] max-w-[150px] flex items-center justify-center gap-1 sm:gap-2 py-2 px-3 sm:py-3 sm:px-4 text-xs sm:text-sm border rounded-xl shadow-sm hover:bg-gray-100 transition"><Thermometer className="text-yellow-500" size={18} /> <span className="font-medium">Temperature</span></button>
            <button onClick={() => openModal("pressure")} className="flex-1 min-w-[70px] sm:min-w-[100px] max-w-[150px] flex items-center justify-center gap-1 sm:gap-2 py-2 px-3 sm:py-3 sm:px-4 text-xs sm:text-sm border rounded-xl shadow-sm hover:bg-gray-100 transition"><Gauge className="text-yellow-500" size={18} /> <span className="font-medium">Pressure</span></button>
            <button onClick={() => openModal("conversion")} className="flex-1 min-w-[70px] sm:min-w-[100px] max-w-[150px] flex items-center justify-center gap-1 sm:gap-2 py-2 px-3 sm:py-3 sm:px-4 text-xs sm:text-sm border rounded-xl shadow-sm hover:bg-gray-100 transition"><MoveHorizontal className="text-yellow-500" size={18} /> <span className="font-medium">Conversion</span></button>
          </div>
        </div>
      </div>

      <MuiModalWrapper open={activeModal !== null} onClose={closeModal} title={activeModal ? modalConfig[activeModal].title : ""}>
