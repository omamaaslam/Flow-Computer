...
const TemperatureDeviceForm: React.FC<TemperatureDeviceFormProps> = observer(
  ({ onBack, onSave, interface_type, device }) => {
    ...
    useEffect(() => {
      ...
      if (device) {
        ...
        let baseState = {
          ...
          temp_min: String(config.temp_min ?? ""),
          temp_max: String(config.temp_max ?? ""),
          ...
          // *** FIX: Correctly merge modbus settings with a default object ***
          modbus_settings: {
            ...formState.modbus_settings, // Start with defaults
            ...(config.modbus_settings || {}), // Merge saved data
          },
          pollingAddress: "",
          variableType: "",
        };
    ...
    const validateAndSave = () => {
      ...
      if (interface_type === "ModbusInterface") {
        // *** FIX: Send slave_address instead of slave_id ***
