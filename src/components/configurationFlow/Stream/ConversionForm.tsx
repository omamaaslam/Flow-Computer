...
const ConversionForm: React.FC<ConversionFormProps> = observer(
  ({ store, config, onCommit, onClose }) => {
    ...
    const handleRowInputChange = (
      ...
      if (field === 'keyboardInput') {
        config.rows[index][field] = parseFloat(value) || 0;
      } else if (field === 'liveValue') {
        const numValue = parseFloat(value);
        config.rows[index][field] = isNaN(numValue) ? value : numValue;
      }
      else {
        (config.rows[index][field] as string) = value;
      }
    };
    ...
    return (
      <div className="flex flex-col gap-4">
        ...
        <div className="border border-gray-200 rounded-md overflow-hidden shadow-sm">
          ...
          <div>
            {config.rows.map((row: ConversionRow, index: number) => (
              <div
                key={row.name}
