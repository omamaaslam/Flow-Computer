...
const MonitorScreen = observer(() => {
  ...
  // Find the specific stream result
  const streamResult = globalStore.results.find(
    (result) => result.stream_id === streamId
  );

  if (!streamResult) {
    return <p>No data available for this stream or stream not found.</p>;
  }
  ...
  const tableDisplayConfig = [
    ...
    // Group 2: Temperature
    {
      ...
      unit: streamResult.temperature_unit,
    ...
    // Group 3: Pressure
    {
      ...
      unit: streamResult.pressure_unit,
              ...
              <div className="border border-gray-200 rounded-lg p-6">
                <p className="text-md">
                  ...
                  <span className="font-semibold text-cyan-600 pl-2">
                    {streamResult.current_volume_original.toFixed(5)} m³
                  </span>
                </p>
              </div>
              <div className="border border-gray-200 rounded-lg p-6">
                <p className="text-md">
                  ...
                  <span className="font-semibold text-cyan-600 pl-2">
                    {streamResult.operating_volume_net.toFixed(5)} m³
                  </span>
                </p>
              </div>
              <div className="border border-gray-200 rounded-lg p-6">
                <p className="text-md">
                  <span className="font-bold text-gray-600">Std Volume</span>{" "}
                  <span className="font-semibold text-cyan-600 pl-2">
                    {streamResult.standard_volume_forward.toFixed(5)} m³
                  </span>
                </p>
              </div>
              <div className="border border-gray-200 rounded-lg p-6">
                <p className="text-md">
                  ...
                  <span className="font-semibold text-cyan-600 pl-2">
                    {streamResult.device_flow_rate.toFixed(5)} m³/h
                  </span>
                </p>
              </div>
              <div className="border border-gray-200 rounded-lg p-6">
          ...
          <div className="grid grid-cols-5 gap-6">
              ...
              <div className="text-sm divide-y divide-gray-200">
                <p className="flex justify-between py-2.5">
                  ...
                  <span className="font-semibold text-cyan-600">
                    {streamResult.operating_pressure.toFixed(5)}{" "}
                ...
                <p className="flex justify-between py-2.5">
                  ...
                  <span className="font-semibold text-cyan-600">
                    {streamResult.operating_temperature.toFixed(5)}{" "}
                ...
                <p className="flex justify-between py-2.5">
                  <span>K-Number:</span>{" "}
                  <span className="font-semibold text-cyan-600">
                    {streamResult.compressibility_k_factor.toFixed(5)}
                  </span>
                </p>
                <p className="flex justify-between py-2.5">
                  <span>Z-Factor:</span>{" "}
                  <span className="font-semibold text-cyan-600">
                    {streamResult.correction_z_factor.toFixed(5)}
                  </span>
                </p>
              </div>
            </InfoCard>
            ...
            <InfoCard title="Alerts" scrollable={true} className="col-span-2">
              <div className="space-y-2 text-sm">
                ...
                >
                  {streamResult.volume_logs?.[
                    streamResult.volume_logs.length - 1
                  ] || "No volume logs"}
                </div>
                <div
                  className={`p-2 border-l-4 ${
                ...
                >
                  {streamResult.compressibility_logs?.[
                    streamResult.compressibility_logs.length - 1
            ...
            <div className="grid grid-cols-3 gap-6">
              ...
              <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-lg flex flex-col items-center justify-around">
                ...
                <div className="text-center">
                  ...
                  />
                  <p className="text-lg text-gray-500 mt-3">
                    {streamResult.operating_temperature}{" "}
                    {streamResult.temperature_unit}
                  </p>
                </div>
              </div>
          ...
          <table className="w-full text-sm text-left">
            ...
            <tbody>
              {tableDisplayConfig.map((item) => {
                ...
                if (!streamResult.hasOwnProperty(item.key)) {
                  return null;
                }

                const value = streamResult[item.key];
                const isInterference = item.statusKey
                  ? streamResult[item.statusKey]
                  : false;
                        ...
                        <div className="flex items-center gap-2">
                          <div
                            className={`w-2.5 h-2.5 rounded-full ${
                              isInterference ? "bg-red-500" : "bg-green-500"
                            }`}
                          ></div>
